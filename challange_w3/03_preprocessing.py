...
def rasterize(img_path: Path) -> np.ndarray | None:
    """
    Convierte una imagen de entrada en un vector de features normalizado.

    Parámetros
    ----------
    img_path : Path
        Ruta a la imagen (jpg, png, webp, etc.).

    Proceso
    -------
    1. Carga la imagen y la convierte a escala de grises (L).
    2. Redimensiona a IMG_SIZE (32 × 32 px) con filtro LANCZOS.
    3. Normaliza los valores de píxel al rango [0, 1].
    4. Aplana la matriz a un vector de 1 024 features.

    Retorna
    -------
    np.ndarray de forma (1024,) con dtype float32, o None si la imagen
    no puede cargarse.

    Ejemplo
    -------
    >>> vec = rasterize(Path("data/spongebob/happy/happy_0001.jpg"))
    >>> vec.shape
    (1024,)
    """
    try:
...
def process_image(img_path: Path, discard_writer) -> np.ndarray | None:
    """
    Llama a rasterize() y aplica filtro de calidad por histograma.
    Descarta imágenes con histograma saturado (>60% en extremos).
    """
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    arr = rasterize(img_path)
    if arr is None:
    ...
    arr_2d = arr.reshape(IMG_SIZE)

    # Verificar histograma — descartar si >60% en extremos
    hist, _       = np.histogram(arr_2d, bins=10, range=(0, 1))
    extreme_ratio = (hist[0] + hist[-1]) / arr_2d.size
...
def plot_sample_faces(X: np.ndarray, y: np.ndarray, n_per_class: int = 5):
    ...
    fig.suptitle("SpongeBob — Muestras procesadas (32×32 px)", fontsize=11)
    ...
    for cls_idx in range(n_classes):
        ...
        for j in range(n_per_class):
            ax = axes[cls_idx, j]
            if j < len(samples):
                ax.imshow(samples[j].reshape(32,32), cmap="gray", vmin=0, vmax=1)
            ax.axis("off")
            if j == 0:
            ...
            for path in images:
                ...
                if vec is not None:
                    ...
                    img_out = Image.fromarray(
                        (vec.reshape(32,32)*255).astype(np.uint8))
